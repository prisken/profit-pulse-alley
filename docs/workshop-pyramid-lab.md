...
| 2026-08-13 | **v5.3 short copy + goal stress signal:** action-goal copy is length-capped (title ≤ 6 EN words / 16 zh-Hant chars; reasoning ≤ 40 EN words / 70 zh-Hant chars) enforced in `parseActionGoalsResult` (over-length → retry → deterministic fallback) + prompt rules + shortened fallbacks; “Estimated rating impact” removed from prose (UI badge shows it). New `goalOutlooks` in the decisions payload: per applied goal — attainedAge, delayYears, requiredExtraMonthlyHKD, effortRatio, `late` (never-reached-by-90 counts as late), `heavyMonthlyCommitment` (needs ≥30% of monthly surplus or surplus ≤ 0). Goal-category goals call the stress out (“Retirement Comfort is never reached by age 90, needing HK$2.5K/mo extra — make the target realistic”). Verified end-to-end on live session `lrd357`. Tests 1286 pass; deployed + pushed `925577c`. |
| 2026-08-13 | **v6 crisis scoring + v5.4 AI directive:** stress-test verdict now measures the dent (`coverageRatio`, `penetrationRatio` = residual ÷ liquid+invested, `investedDrawdownPct`) — SHIELDED needs cover ≥60% + dent ≤15% + no delay + drawdown ≤10%; PENETRATED needs cover <40%, dent >60%, delay ≥2 yrs, or heavy drawdown with weak cover; scaled drawdown penalty replaces flat −6; PARTIAL band 45–78. Live session `lrd357` (75% cover, HK$60k residual, 13% drawdown): PENETRATED/29 → **PARTIAL/62**. Action-goal directive: `dataGaps[]` (missing/zero inputs, bilingual, severity) → AI may add ONE short refine ask ("Tell us your actual {label} for a precise figure") when the gap matches the goal category, never inventing numbers; realism rules cap asks at `remainingMonthlySurplus` and prefer "make the target realistic" over "save harder"; deterministic fallback mirrors the refine note without step-number leaks. Tests 1290 pass; deployed + pushed `5ed4fc3`. |
| 2026-08-13 | **Pitch Meeting shipped + AdminNav:** ported the Pitch Meeting game (`/pitch`, bilingual investor-pitching game with DeepSeek reaction + deterministic fallback), `/admin/pitch` leads table (CSV export), `PitchMeetingLead` model, and the persistent `AdminNav` bar on every `/admin` route (Dashboard / Market Pulse / Matching Pulse / Pitch Meeting leads / Workshop leads) — from the alley clone's local commits (built Aug 9, never deployed). No admin console is orphaned anymore (MP approvals + cycles + workshop leads all reachable). Fixed alley lint debt (`module` var names, unused import). Tests 1294 pass; deployed + pushed `10de630`. |
| 2026-08-13 | **Admin lead detail + anonymous capture (v7):** both lead consoles now open the FULL session on click — `/admin/workshop` rows expand to a visual pyramid diagram (final vs AI-predicted toggle, layer values, expenses, goals + applied/given-up chips, risk quiz, crisis verdict) ported from the alley clone's `WorkshopSessionDetail` and adapted to v5 shapes (lump sum + risk allocation, `computeLayerFlags` 2-arg); `/admin/pitch` rows expand to journey/readout/raw JSON. Whole rows are clickable (not just the chevron). Contact capture is now OPTIONAL in both games — players who skip name/email/phone still land as leads with full game data; admin shows "Anonymous player" / "—" proxies (stored as ""), CSV exports blank cells. Conditional validation kept (non-empty fields still validated). Tests 1298 pass; deployed + pushed. |
