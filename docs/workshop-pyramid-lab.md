...
| **Version** | v4 (goal journey rail + squeeze + retirement finale; life timeline + crisis engine + rating/PDF/admin) |
...
4. Run a **life-timeline stress test** with a **goal journey**: overview donuts → age-sequenced rail (Apply / Give up per spend goal, optional spending squeeze) → retirement finale charts + decision recap (deterministic `runLifeTimeline` + AI notes only for amber/red/**oversaved** / squeeze narration).
...
| **stresstest** | `WorkshopStressTestStep` | **(1)** Overview dual large `WorkshopAllocationDonut`s (`WorkshopGoalJourneyOverview` + surplus intro). **(2)** Headline coverage / depletion cards + EF collapsible (AI notes on amber/red/oversaved). **(3)** `WorkshopGoalJourneyRail` — age-sequenced spend goals then retirement; lock until earlier decisions; card interiors = outlook + liquidation toggle + compact Current/Recommended squeeze donuts + Apply/Give up. **(4)** Retirement finale auto-expands when all spend goals decided: remounted dual cash-flow/asset **line** charts + age scrubber (`WorkshopRetirementFinaleCharts`) + decision recap. Sticky footer **Continue to Risk Quiz** enables only after finale unlocked. |
...
| `investment` | Investment & Fun | `InvestmentLayerEditor` | Risk L/M/H (sum 100), **`lumpSumHKD`** (current invested capital), **`monthlyInvestmentHKD`** (monthly contribution during working years), monthly fun. |

**v3 goal timing:** `GoalItem.targetAge` is SSOT; `targetYear = deriveGoalYear(targetAge, userAge)` in `goal-year.ts`. Old sessions with only `targetYear` derive `targetAge` on parse. Legacy goals without `goalType` default to `"spend"`.
...
**Life timeline engine (v4):** `src/lib/workshop/timeline-engine.ts` — `runLifeTimeline` / `goalStatusAtYear`. Pure TS year-by-year to age 90 (working salary via existing `advanceMonthlyIncomeForYear`; surplus → **capped monthly investing** into invested + remainder to liquid; spend goals pay liquid first and **only liquidate invested when `allowLiquidation` is true**; `retirementTarget` goals never deducted — reported as `retirementTargets`; retired salary 0 + passive = invested × blended rate; drawdown liquid→invested; EF `oversaved` at 1.5× target). Wizard stress step calls `runLifeTimelineAction`. Goal-journey re-runs use `rerunTimelineForJourney` so excluded goals and accepted squeezes feed the same concurrent engine path. Legacy `runGoalStressTest` remains for tests / old sessions (§9).
...
- **Stress-test / goal journey (v4):** Step opener is dual **`WorkshopAllocationDonut`** overview (cash-flow allocation + today's liquid/invested) via `WorkshopGoalJourneyOverview` + `deriveJourneyOverview`; framing copy `workshop.stressTest.journeyIntro` cites monthly surplus. Spend goals are decided on the **journey rail**; dual cash-flow/asset **line** charts + age scrubber live only in `WorkshopRetirementFinaleCharts` on the retirement finale card — timeline data still computed on the step.
...
**`WorkshopGoalJourneyRail`** (`src/components/workshop/WorkshopGoalJourneyRail.tsx`) — vertical age-rail of goal `CollapsibleWidget`s (spend by `targetAge`, retirement always last). Earlier unresolved decisions lock later cards; first pending auto-expands; resolved chips (on track / delayed / given up); reopening an earlier resolved goal nudges later ones with “may need to revisit”. Spend interiors: `WorkshopGoalJourneyCard`. Retirement finale: `WorkshopGoalJourneyFinaleCard` remounts `WorkshopRetirementFinaleCharts` + decision recap; sticky footer Continue enables only once all spend goals are decided.
...
| Investing flow | Working years: `investedContribution = min(monthlyInvestmentHKD × 12, max(0, surplus))` → invested; remainder → liquid. Stops at retirement. |
...
| Retirement targets | `TimelineResult.retirementTargets`: nest-egg target (today's HKD) vs `assetsAtRetirement` → `gapHKD` / `met` |
| Working years | Salary via career curve; expenses + fun flat; surplus split as above; liquid decays; invested compounds at real blend |
| Retirement | Salary **0**; **passive income = invested × blended real rate**; drawdown order **liquid → invested**; monthly investing stops; liquid still decays |
...
- **`goalsOnTrack`** = **85%** goal flags (spend: green=1 / amber=0.5 / red=0 / **given_up=`GIVEN_UP_GOAL_CREDIT` (0.6)**; **retirementTarget**: met=1, gap ≤20%=0.5, else 0) + **15%** retirement readiness from `assetsDepletedAtAge` (`null` = sustained past 90 → full marks)
...
4. **Your Plan Decisions** (when `goalJourneyJson` has resolved spend goals): on-time / delayed / given-up counts + chip list + optional monthly plan before→after (same recap as the retirement finale)
5. **Retirement snapshot** (when timeline / `retirementAge` present): planned age, passive income at retirement (**annual**), assets at retirement, **passive coverage %** at retirement, nest-egg target vs projected (met/gap), depleted-at-age or “sustained”
6. **Goals:** spend goals — inflated targets + attained age + progress bars; **retirementTarget** goals — target-vs-projected line (no progress bar); labels from `Bilingual`
...
| File | Covers |
|------|--------|
| `timeline-engine.test.ts` | `runLifeTimeline`, contribution capped by surplus, remainder to liquid, spend-goal liquidation gated by `allowLiquidation`, retirementTargets gap math, high-saver no false depletion, EF oversaved (1.5×), `goalStatusAtYear`, `assetsDepletedAtAge` |
...
| `WorkshopGoalJourneyRail.test.tsx` | Lock sequencing, squeeze donuts in active card, Apply unlocks next, retirement finale charts + recap when spend goals decided |
...
| `pyramid-benchmarks.test.ts` | Benchmarks + layer flags + monthly investing over-surplus amber |
| `risk-quiz.test.ts` | 5Q scoring → profile |
| `financial-rating.test.ts` | Weights, oversaved EF (88), goals 85/15 + retirement readiness, retirementTarget met/gap, **given_up = 0.6 not red**, crisisResilience from `impactResult`, impact points |
...
3. Confirm → expenses → stress test:
   ...
   - Sticky footer **Continue to Risk Quiz** stays disabled until retirement finale unlocks.
   - Finale scrubber: parent `touch-pan-y`, scrubber `touch-none`, tap-to-jump (unchanged v3.2 rules).
4. Risk quiz → crisis impacts → summary rating → pick #1 → capture with **required phone** → PDF includes **Your Plan Decisions**.
