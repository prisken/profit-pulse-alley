...
export const metadata = {
  title: "Blog（中文）| Profit Pulse Ally",
};

export default async function BlogZhHkIndexPage() {
  const posts = await getAllPosts("zh-hk");
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
      <header className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          <Link href="/blog/en" className="text-foreground/70 hover:underline">
            English
          </Link>
        </div>
      </header>
        ...
        {posts.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {posts.map((post) => (
              ...
              >
                <Link href={`/blog/zh-hk/${post.slug}`} className="block">
