...
import MarketPulseGuidedLaunchClient from "@/components/admin/MarketPulseGuidedLaunchClient";
...
import {
  ...
  buildAdminMpGuidedLaunchBreadcrumbs,
...
export async function generateMetadata({
  ...
  const data = await getGuidedCycleLaunchPageData(cycleId);

  return {
    title: translate(locale, "auth.meta.adminMpGuidedLaunch.title"),
    description: data
      ? translateWith(locale, "auth.meta.adminMpGuidedLaunch.description", {
          cycle: data.cycle.name,
        })
      : translate(locale, "auth.meta.adminMpGuidedLaunch.descriptionFallback"),
  };
}

export default async function MarketPulseGuidedLaunchPage({
  params,
}: {
  params: Promise<{ cycleId: string }>;
}) {
      ...
      <div className="mx-auto w-full max-w-7xl overflow-x-hidden px-3 py-6 sm:px-6 sm:py-12">
        <header className="border-b border-zinc-800 pb-6">
          <MarketPulseAdminBreadcrumbs
            items={buildAdminMpGuidedLaunchBreadcrumbs(data.cycle.name)}
          />
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-zinc-50 sm:text-3xl">
                {t("auth.admin.mp.guidedLaunch.title")}
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
                {t("auth.admin.mp.guidedLaunch.subtitle")}
              </p>
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href={marketPulseGuidedCardsPath(cycleId)}
        ...
        <div className="mt-6 sm:mt-8">
          <MarketPulseGuidedLaunchClient initialData={data} />
        </div>
      </div>
    </main>
  );
