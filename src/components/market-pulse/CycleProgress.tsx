    ...
    <div className={className}>
      {showLabel ? (
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="text-sm font-semibold text-emerald-200">{displayLabel}</p>
        </div>
      ) : null}

      <div
        className={`overflow-hidden rounded-full bg-zinc-800/90 ring-1 ring-zinc-700/50 ${showLabel ? "mt-2" : ""}`}
        role="progressbar"
      ...
      >
        <motion.div
          className="relative h-2 overflow-hidden rounded-full sm:h-2.5"
          initial={{ width: 0 }}
